...
describe('MidiAccessService', () => {
  ...
  describe('claims', () => {
    it('grants a claim on an unheld port and reports the holder', () => {
      ...
      expect(service.holderOf('port-1')).toBe('A');
    });

    it('refuses a claim on a port another holder already holds, in both directions', () => {
      expect(service.claim('A', 'port-1')).toBe(true);
      ...
      expect(service.holderOf('port-2')).toBe('B');
    });

    it('releases the claiming holder\'s previous claim once a new claim succeeds', () => {
      service.claim('A', 'port-1');

      service.claim('A', 'port-2');

      expect(service.holderOf('port-1')).toBeNull();
      expect(service.holderOf('port-2')).toBe('A');
    });

    it('release() drops the named holder\'s claim without touching another holder\'s', () => {
      service.claim('A', 'port-1');
  ...
  describe('outputPortFor', () => {
    it('returns null when portId names no currently enumerated output', () => {
      expect(service.outputPortFor('missing')).toBeNull();
    });

    it('wraps the named output so it can send and report supportsCancel', async () => {
      const output = makeOutput('port-1', 'A', 'Acme', true);
      stubRequestMidiAccess(() => Promise.resolve(makeAccess([output])));
      await service.requestAccess();

      const port = service.outputPortFor('port-1');

      expect(port?.portId).toBe('port-1');
      expect(port?.supportsCancel).toBe(true);
      port?.send(Uint8Array.from([0xf0]));
      expect(output.send).toHaveBeenCalledWith(Uint8Array.from([0xf0]), undefined);
    });

    it('memoises the wrapped port on the underlying output object\'s identity', async () => {
      const output = makeOutput('port-1', 'A', 'Acme');
      stubRequestMidiAccess(() => Promise.resolve(makeAccess([output])));
      await service.requestAccess();

      const first = service.outputPortFor('port-1');
      const second = service.outputPortFor('port-1');

      expect(first).not.toBeNull();
      expect(second).toBe(first);
    });

    it('re-wraps rather than reusing the stale wrapper when a same-id reconnect replaces the output object', async () => {
      const original = makeOutput('port-1', 'A', 'Acme');
      const access = makeAccess([original]);
      stubRequestMidiAccess(() => Promise.resolve(access));
      await service.requestAccess();
      const beforeReconnect = service.outputPortFor('port-1');

      const replacement = makeOutput('port-1', 'A', 'Acme');
      access.outputs.set('port-1', replacement);
      access.onstatechange?.({});

      const afterReconnect = service.outputPortFor('port-1');
      afterReconnect?.send(Uint8Array.from([0xf0]));
