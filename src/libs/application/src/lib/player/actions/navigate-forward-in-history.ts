...
export function navigateForwardInHistory(
  ...
  return {
    navigateForwardInHistory: async (params: NavigateForwardInHistoryParams): Promise<void> => {
      ...
      logInfo(
        LogType.Start,
        `NavigateForwardInHistory: Starting forward navigation for device ${deviceId}`,
        {
          deviceId,
          actionMessage,
        }
      ...
      if (!history || history.entries.length === 0) {
        logInfo(
          LogType.Info,
          `NavigateForwardInHistory: No history for device ${deviceId}, cannot navigate forward`
        );
        return;
      }
      ...
      if (currentPosition === -1) {
        logInfo(
          LogType.Info,
          `NavigateForwardInHistory: Already at end (-1) for device ${deviceId}, cannot go forward`
        );
        return;
      }
      ...
      if (targetPosition >= history.entries.length) {
        logInfo(
          LogType.Info,
          `NavigateForwardInHistory: Target position ${targetPosition} beyond history length ${history.entries.length} for device ${deviceId}`
        );
        return;
      }
      ...
      if (!historyEntry) {
        logError(
          `NavigateForwardInHistory: No entry at position ${targetPosition} for device ${deviceId}`
        );
        return;
      }
      ...
      logInfo(
        LogType.Info,
        `NavigateForwardInHistory: Moving from position ${currentPosition} to ${targetPosition}`
      );
      logInfo(
        LogType.Info,
      ...
      try {
        ...
        logInfo(
          LogType.Success,
          `NavigateForwardInHistory: File ${launchedFile.name} launched successfully`
        );
        ...
        logInfo(
          LogType.Finish,
          `NavigateForwardInHistory: Forward navigation completed for device ${deviceId}. New position: ${targetPosition}`
        );
      } catch (error) {
