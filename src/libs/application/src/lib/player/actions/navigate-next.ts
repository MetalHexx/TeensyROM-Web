...
export function navigateNext(store: WritableStore<PlayerState>, playerService: IPlayerService, playerStorage: IPlayerStorage) {
  return {
    navigateNext: async ({ deviceId }: { deviceId: string }): Promise<void> => {
      ...
      logInfo(LogType.Start, `Navigating to next file for ${deviceId}`, {
        deviceId,
        actionMessage,
      });
      ...
      try {
        if (launchMode === LaunchMode.Shuffle) {
          ...
          if (!isCompatible) {
            ...
            logError(
              `Navigate next: Random file ${launchedFile.name} is incompatible with device ${deviceId}: ${errorMessage}`
            );
            setShuffleNavigationFailure(
              store,
              deviceId,
