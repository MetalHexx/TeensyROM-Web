...
- Clean working directory (commit/stash changes)

### Step 1: Build Frontend Production

```bash
# From repository root (src/)
...
3. Copies build output to wwwroot
...
# Should see: index.html, *.js, *.css, favicon.ico, images, etc.
```
...
3. Check browser DevTools Network tab
   ...
   - API calls go to `/api/*` (e.g., `/api/devices`)
   - SignalR connects to `/logHub`, `/deviceEventHub`

**Common issues**:
- ❌ **404 on root**: wwwroot not populated (run `pnpm run build:frontend`)
- ❌ **API 404s**: Missing `/api/` prefix (Phase 01 not complete)
- ❌ **CORS errors**: Check `appsettings.json` has correct allowed origins

### Step 3: Test Windows x64 Publish
...
- `-p:SkipBuildFrontend=true`: Don't run copy-frontend.ps1 (already built)
...
```
publish/win-x64/
├── TeensyRom.Api.exe     (Single executable, ~122 MB)
├── TeensyRom.Api.pdb     (Debug symbols)
└── web.config             (IIS config - can be deleted for standalone)
```
...
**Common issues**:
- ❌ **"wwwroot not found"**: Frontend wasn't built before publish (run Step 1)
- ❌ **Asset loading failures**: `AppContext.BaseDirectory` not used (Phase 03 issue)
- ❌ **Composer images 404**: Base URL hardcoded (Phase 01 issue)
...
./publish/osx-arm64/TeensyRom.Api
```
...
./publish/linux-x64/TeensyRom.Api
```
...
```bash
cd publish/win-x64
Compress-Archive -Path * -DestinationPath ../TeensyROM-Web-test-win-x64.zip
```

**macOS/Linux (.tar.gz)**:
```bash
cd publish/osx-x64
tar -czvf ../TeensyROM-Web-test-osx-x64.tar.gz .

cd ../osx-arm64
tar -czvf ../TeensyROM-Web-test-osx-arm64.tar.gz .

cd ../linux-x64
tar -czvf ../TeensyROM-Web-test-linux-x64.tar.gz .
```

**Verify archives**:
1. Extract archive to new location
2. Run executable
