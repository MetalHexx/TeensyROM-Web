# Task: Fix DeviceLogsService Tests
...
**What**: Add the missing `API_CONFIG` injection token provider to the DeviceLogsService test suite.
...
**Files to Modify**:
- `libs/infrastructure/src/lib/device/device-logs.service.spec.ts` - Add API_CONFIG provider

**Files to Review** (for context):
- `libs/infrastructure/src/lib/config/api-config.provider.ts` - API_CONFIG token and interface
- `libs/infrastructure/src/lib/device/device-logs.service.ts` - Service dependencies
...
**Validation Steps**:
1. Run `pnpm exec nx run infrastructure:vite:test -- --testPathPattern=device-logs` - all tests should pass
...
Save completion report to: `reports/TEST-REPAIR-TASK-02-001-REPORT.md`

---

*Task 1 of 2 in Phase 2 | Est. Time: 15 minutes*
