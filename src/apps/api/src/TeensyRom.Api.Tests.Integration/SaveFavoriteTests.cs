...
{
    [Collection("Endpoint")]
    public class SaveFavoriteTests(EndpointFixture f)
    {
            ...
            string filePath, string expectedFavoritePath)
        {
            ...
            r.Content.Message.Should().NotBeNullOrEmpty().And.Contain("Favorite tagged and saved to").And.Contain(expectedFavoritePath);
        ...
        public async void When_SaveFavoriteCalled_WithDeviceThatDoesntExist_ReturnsNotFound()
        {
            ...
            var r = await f.Client.PostAsync<FavoriteFileEndpoint, SaveFavoriteRequest, string>(request);
